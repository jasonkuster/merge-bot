...
class MergeTest(unittest.TestCase):
    ...
    def get_gitmerger_for_testing(self, mock_logger):
        q = MagicMock()
        pipe = MagicMock()
    ...
    @patch('mergebot_backend.db_publisher.publish_dequeue')
    @patch('mergebot_backend.db_publisher.publish_merger_status')
    def test_fetch_terminate(self, mock_status, mock_dequeue, mock_pr):
        ...
        self.assertEqual(mock_dequeue.call_count, 1)

    @patch('mergebot_backend.github_helper.GithubPR')
    @patch('mergebot_backend.db_publisher.publish_dequeue')
    def test_fetch(self, mock_dequeue, mock_pr):
        gm = self.get_gitmerger_for_testing()
        gm.pipe.poll.return_value = False
        gm.work_queue.get.side_effect = Queue.Empty
        self.assertIsNone(gm.fetch_from_queue())
        self.assertEqual(mock_dequeue.call_count, 0)
        q = MagicMock()
    ...
    def test_verify_jenkins_failure_success(
            self, mock_job, mock_pr, mock_logger, pis, pih, mock_build):
        gm = self.get_gitmerger_for_testing()
